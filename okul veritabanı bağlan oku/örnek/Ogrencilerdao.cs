...
namespace örnek
{
    class Ogrencilerdao
    {
        ...
        {
            ArrayList ogrencitorbasi = new ArrayList();
            ...
            while (oku.Read())
            {
               ...
               ogrencitorbasi.Add(ogr);
            }
            baglanti.Close();
            return ogrencitorbasi;

        }

        public Ogrenci istenilenogrencigetir(int ogno)
        {
            baglanti.Open();
            komut.Connection = baglanti;
        ...
        public int ogrencikaydet(int ogno, String adsoyad, long tckimlik, DateTime dogtar, String adres)
        {
            ...
            try
            {
                baglanti.Open();
                komut.Connection = baglanti;
komut.CommandText = "insert into ogrenciler (ogno,ogadsoyad,dogtar,tckimlik,adres) values ("
  + ogno + ",'" + adsoyad + "',convert(datetime,'" + dogtar + "',103)," + tckimlik
            + ",'" + adres + "')";
...
public int ogrencidegistir(int ogno, String adsoyad, long tckimlik, DateTime dogtar, String adres)
        {
            baglanti.Open();
            komut.Connection = baglanti;
            komut.CommandText = "update ogrenciler set ogadsoyad='" + adsoyad + "',tckimlik=" +
                tckimlik + ",dogtar=convert(datetime,'" + dogtar + "',103),adres='" + adres 
                + "' where ogno=" + ogno;
            int sayi = komut.ExecuteNonQuery();
            baglanti.Close();
            return sayi;
